...
# DGM based solver for Kdv equation
...
#%% Set up network
#model = DDM.DeepNet(nodes_per_layer, num_layers, dim)
model = net.DeepNet(nodes_per_layer, num_layers, dim,final_trans="tanh")
...
# loss
L1_tnsr, L2_tnsr, L3_tnsr = ls.loss(model, t_e_tnsr, x_e_tnsr, t_i_tnsr, x_i_tnsr, t_b_tnsr, x_b_tnsr, u_initial, u_boundary).kdv()
...
for i in range(sampling_stages):
    ...
    for _ in range(steps_per_sample):
        ...
        if step%100 == 0:
            ...
            predict_sol = sess.run([V], feed_dict = {t_e_tnsr:nodes[:,1].reshape(-1,1), x_e_tnsr:nodes[:,0].reshape(-1,1)})[0]
            exact_sol = ue(XX,TT)
            exact_sol = np.reshape(exact_sol.T,[-1,])
            er = predict_sol.T - exact_sol
...
plt.contourf(XX,TT,predict_sol.reshape(N,N).T,cmap=MAP, norm=norm)
...
plt.title('DGM error',fontsize=ftsize)
plt.colorbar()
plt.subplot(2,2,4)
plt.tick_params(labelsize='large')
it = np.linspace(0,2500,25)
