...
# DGM based solver for hign dimensional heat equation
...
def ssin(features, name=None):
    return tf.sin(features)

# initialize Model
final_s = linear
model = net.DeepNet(nodes_per_layer, num_layers, dim,final_trans=final_s)
...
L1_DGM, L2_DGM, L3_DGM = ls.loss(model, t_e_tnsr, x_e_tnsr, t_i_tnsr, x_i_tnsr, t_b_tnsr, x_b_tnsr, u_initial, u_boundary).Heat_ND(f, dim, l1, l2)
loss_DGM = L1_DGM + L2_DGM + L3_DGM
...
for i in range(sampling_stages):
    ...
    for _ in range(steps_per_sample):
        t1 = time.time()
        loss, L1, L2, L3, _ = sess.run([loss_DGM, L1_DGM, L2_DGM, L3_DGM, optimizer_DGM],
        ...
        if step%1000 == 0:
            ...
            DGM_sol = sess.run([V], feed_dict = {t_e_tnsr:np.reshape(TT,[(N+1)**2,1]), x_e_tnsr:x})[0]
            er = np.reshape(DGM_sol,[-1,]) - exact_sol
            error_record.append(np.linalg.norm(er)/np.linalg.norm(exact_sol))
            data_record.append(DGM_sol)
...
plt.subplot(1,3,1)
plt.tick_params(labelsize='large')
plt.semilogy(it,loss_record,'r',label='DGM')
#plt.legend()
plt.xlabel('iteration steps',fontsize=ftsize)
plt.ylabel('loss',fontsize=ftsize)
plt.subplot(1,3,2)
plt.tick_params(labelsize='large')
plt.plot(it, time_record,'r',label='DGM')
#plt.legend()
plt.xlabel('iteration steps',fontsize=ftsize)
plt.ylabel('CPU time/s',fontsize=ftsize)
plt.subplot(1,3,3)
plt.tick_params(labelsize='large')
plt.semilogy(time_record,error_record,'r',label='DGM')
plt.legend()
